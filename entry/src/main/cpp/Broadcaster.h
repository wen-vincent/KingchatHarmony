...
public:
    struct TimerKiller {
        ...
        template <class R, class P> bool WaitFor(std::chrono::duration<R, P> const &time) const {
...
public:
    ...
    std::future<std::string> OnProduce(mediasoupclient::SendTransport * /*transport*/, const std::string &kind,
...
public:
    const nlohmann::json &Start(bool enableAudio, bool useSimulcast, const nlohmann::json &routerRtpCapabilities,
                                bool verifySsl = true);
    void Stop();
