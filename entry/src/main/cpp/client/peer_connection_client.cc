...
rtc::Socket *CreateClientSocket(int family) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, "mytest", "CreateClientSocket");
...
PeerConnectionClient::PeerConnectionClient() : callback_(NULL), resolver_(nullptr), state_(NOT_CONNECTED), my_id_(-1) {}

PeerConnectionClient::~PeerConnectionClient() = default;

void PeerConnectionClient::InitSocketSignals() {
    RTC_LOG(LS_INFO) << __FUNCTION__;
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, "mytest", "InitSocketSignals");
    RTC_DCHECK(control_socket_.get() != NULL);
    RTC_DCHECK(hanging_get_.get() != NULL);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, "mytest", "InitSocketSignals 11111111111");
...
int PeerConnectionClient::id() const { return my_id_; }

bool PeerConnectionClient::is_connected() const { return my_id_ != -1; }

const Peers &PeerConnectionClient::peers() const { return peers_; }
...
void PeerConnectionClient::Connect(const std::string &server, int port, const std::string &client_name) {
    ...
    RTC_LOG(LS_INFO) << server << ":" << port << ", client name : " << client_name;
    ...
    RTC_LOG(LS_INFO) << __func__ << __LINE__;
...
void PeerConnectionClient::OnResolveResult(const webrtc::AsyncDnsResolverResult &result) {
...
void PeerConnectionClient::DoConnect() {
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, "mytest", "DoConnect control_socket_ & hanging_get_");
...
bool PeerConnectionClient::SendHangUp(int peer_id) { return SendToPeer(peer_id, kByeMessage); }
...
bool PeerConnectionClient::GetHeaderValue(const std::string &data, size_t eoh, const char *header_pattern,
                                          size_t *value) {
...
bool PeerConnectionClient::ReadIntoBuffer(rtc::Socket *socket, std::string *data, size_t *content_length) {
    char buffer[0xffff];
    do {
...
bool PeerConnectionClient::ParseServerResponse(const std::string &response, size_t content_length, size_t *peer_id,
                                               size_t *eoh) {
